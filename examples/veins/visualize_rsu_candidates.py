...
lats = [x[1] for x in rsu_results]
lons = [x[2] for x in rsu_results]
degrees = [x[3] for x in rsu_results]

plt.figure(figsize=(12, 10))
scatter = plt.scatter(lons, lats, s=[d**3.5 for d in degrees], c=degrees, 
                      cmap='plasma', alpha=0.6, edgecolors='white', linewidth=1.5)

for i, (name, lat, lon, deg, dens) in enumerate(rsu_results):
    plt.text(lon, lat + 0.0003, f'#{i}', fontsize=9, fontweight='bold', ha='center')

plt.colorbar(scatter, label='Junction Degree (Connectivity)')
plt.title('Optimization of RSU Placement in Erlangen', fontsize=16)
plt.xlabel('Longitude')
plt.ylabel('Latitude')
plt.grid(True, linestyle=':', alpha=0.6)
plt.tight_layout()
plt.savefig("rsu_spatial_analysis.png", dpi=300)
plt.show()

m = folium.Map(location=[49.575, 11.015], zoom_start=14, tiles='CartoDB positron')

for i, (name, lat, lon, deg, dens) in enumerate(rsu_results):
    folium.CircleMarker(
        location=[lat, lon],
        radius=deg * 1.5,
        popup=f"RSU {i}<br>ID: {name}<br>Degree: {deg}<br>Density: {dens}",
        color="#e74c3c",
        fill=True,
        fill_color="#e74c3c",
        fill_opacity=0.7
...
m.save("rsu_location_interactive_map.html")
