...
for row in rsu_results:
    ...
    sim_x = utm_x - OFFSET_X
    sim_y = utm_y - OFFSET_Y
    
    dummy_pdr = (row[3] * 10) + row[4] 
    
    csv_data.append([node_id, lat, lon, sim_x, sim_y, dummy_pdr])

# 3. 데이터프레임으로 변환 후 CSV로 저장 (기존 파일 덮어쓰기)
df = pd.DataFrame(csv_data, columns=['ID', 'Lat', 'Lon', 'Sim_X', 'Sim_Y', 'PDR'])
df.to_csv("rsu_pdr_optimization_results_40.csv", index=False)

print("="*60)
print("✅ 소영님 맵 기준(Offset)으로 40개 교차로 좌표 보정 및 CSV 저장 완료!")
print("="*60)
