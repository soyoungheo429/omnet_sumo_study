...
# =========================================================
# 5. 메인: 다중 실행(Multi-Run) 제어 및 통계 수집
# =========================================================
NUM_RUNS = 7  # 10회 또는 30회로 설정

print("\n" + "="*60)
print(f"[Robustness Test] MS-ASA (논문 수식 적용) {NUM_RUNS}회 독립 실행 시작!")
print("="*60)
...
for run_idx in range(1, NUM_RUNS + 1):
    ...
    # [매우 중요] 각 Run마다 다른 난수를 발생시키되, 실험 재현성을 위해 시드 부여
    random.seed(42 + run_idx)
    
    # 1. 이번 Run을 위한 초기 온도 동적 계산 (독립성 보장)
    ...
    # 3. 이번 Run의 에이전트 기록 통합 및 Step별 통계 압축
...
# 1. Step별 통계 계산 (NUM_RUNS 회의 결과를 모아 평균, 중앙값, 표준편차 도출)
...
# 2. [최고 PDR] 평균선 및 에러 음영 (빨간색 실선, ±1σ)
...
plt.title(f"MS-ASA (Paper Params) Robustness ({NUM_RUNS} Independent Runs)", fontsize=14)
plt.legend(loc='lower right', fontsize=10)
plt.grid(True, linestyle='--', alpha=0.6)
plt.tight_layout()

plt.savefig("ms_asa_paper_robustness_errorbar.png", dpi=300)
print("\n=> 'ms_asa_paper_robustness_errorbar.png' 논문용 신뢰도 차트 저장 완료!")
