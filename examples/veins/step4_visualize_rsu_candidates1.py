...
m = folium.Map(location=[49.577, 11.015], zoom_start=14, tiles='CartoDB Positron') 

for i, (name, lat, lon, deg, dens) in enumerate(rsu_results):
    marker_color = "darkblue" if deg >= 6 else "blue" if deg >= 4 else "lightblue"
    
    folium.Marker(
        location=[lat, lon],
        popup=f"ID: {name}<br>Degree: {deg}<br>Density: {dens}",
...
m.save("erlangen_candidates_overlap_map.html")
